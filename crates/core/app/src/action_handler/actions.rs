...
impl AppActionHandler for Action {
    ...
    async fn check_stateless(&self, context: TransactionContext) -> Result<()> {
        match self {
            ...
            Action::TokenBurn(action) => action.check_stateless(()).await,
    ...
    async fn check_historical<S: StateRead + 'static>(&self, state: Arc<S>) -> Result<()> {
        match self {
            ...
            Action::TokenBurn(action) => action.check_historical(state).await,
    ...
    async fn check_and_execute<S: StateWrite>(&self, state: S) -> Result<()> {
        match self {
            ...
            Action::TokenBurn(action) => action.check_and_execute(state).await,
